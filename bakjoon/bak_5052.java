    ...
    static class Tree {
        ...
        public boolean addString (String string) {
            ...
            for (int i = 0; i < string.length(); ++i) {
                ...
                if (!pointer.hasChild(data)) { // 여기는 무조건 새 데이터.
                    pointer.addChild(data, node);
                    pointer = pointer.getChild(data);
                    continue;
                }

                pointer = pointer.getChild(data); // 여기는 기존에 있던 데이터.
                if (pointer.isEnd()) {
                    return false;
                }
                if (i + 1 == string.length()) { // 91125426 -> 911 입력시 틀려야 함; 입력은 순서가 있지만, 사용에는 순서가 없으니, 나중에 추가되는 번호가 접두어여도 안 되는 것.
                    return false;
                }
            }
